...
Redis 默认的方式, 通过以指定的时间间隔执行数据集的时间点快照来将数据持久化到磁盘中. Redis 会单独创建(fork)一个子进程来进行持久化, 会先将数据写入到一个临时文件中, 待持久化过程都结束了, 再用这个临时文件替换上次持久化的文件. 整个过程中, 主进程是不进行任何 IO 操作的, 这样确保了极高的性能.
...
Redis 默认是不使用该方式持久化的. AOF 方式的持久化, 是操作一次 Redis 数据库, 则将操作的记录存储(不包括查询操作)到 AOF 持久化文件中, 以日志的形式来记录每个写操作. 
